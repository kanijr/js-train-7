...
function useSymbolDescription() {
  ...
  const symbolDescritpion = mySymbol.description;
  return symbolDescritpion;
}
...
let rangeObject = {
  ...
  [Symbol.iterator]() {
    this.currect = this.from;
    return this;
  },

  next() {
    if (this.currect < this.to) {
      return { done: false, value: this.currect++ };
    } else {
      return { done: true };
    }
  },
...
let myObject = {
  ...
  next() {
    return this.currentIndex < this.category.length
      ? { value: this.category[this.currentIndex++].name, done: false }
      : { done: true };
  },
};
